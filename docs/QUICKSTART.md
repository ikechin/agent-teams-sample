...
このプロジェクトは**Gitサブモジュール**を使用しています。各サービス（Frontend/BFF/Backend）は独立したGitリポジトリです。

**新しくクローンした場合、またはservices/が空の場合:**
...
# 以下のように表示されればOK:
# 0421985... services/backend (heads/main)
# ad4296f... services/bff (heads/main)
# b37ed2a... services/frontend (heads/main)
```
...
**引数:** ステアリングディレクトリ名（例: `20250407-frontend-bff-only`）
...
2. 必要なドキュメントを読み込み
3. タスクスコープを要約して確認
4. Agent Teamsプロンプトを生成・実行

**メリット:**
- ✅ プロンプトの揺れを防止
- ✅ タスク間違いを防止（明示的に指定）
- ✅ 開発メンバー全員が同じ手順で実装開始
- ✅ 複数のステアリングディレクトリがあっても安全
