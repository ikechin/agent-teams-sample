...
test.describe.serial('承認履歴検索 E2E', () => {
  ...
  test.beforeAll(async ({ browser }) => {
    ...
    // ---------------------------------------------------------------
    // setup project (auth.setup.ts) が全ロールの storageState を生成済み。
    // API/browser context はすべて storageState を流用するので、本スペック
    // での login 呼び出しはゼロ (rate limit 消費なし)。
    // ---------------------------------------------------------------
